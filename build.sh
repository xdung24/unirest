...
CGO_ENABLED=0 GOOS=$GO_OS GOARCH=$GO_ARCH go build -o ./tmp/main.exe -x --ldflags="-X 'main.GoOs=$GO_OS' -X 'main.GoArch=$GO_ARCH' -X 'main.GoVersion=$GO_VERSION' -X 'main.AppVersion=$APP_VERSION' -X 'main.GitHash=$GIT_HASH' -X 'main.GinVersion=$GIN_VERSION' -X 'main.GorillaVersion=$GORILLA_VERSION' -X 'main.EchoVersion=$ECHO_VERSION' -X 'main.FiberVersion=$FIBER_VERSION'" | true